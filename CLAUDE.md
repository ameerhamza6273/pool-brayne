...
- **2026-09-25 (client video+docx "Jobs.dispatch.notes", SMS list #1, SMS #2 Timesheets)** — built, NOT yet pushed (dev wants a joint test first): POS cart price edit, right-click price = cost, editable receipt disclaimer (`tenants.receipt_disclaimer`) + real printed receipt (`src/lib/pos-receipt.ts`), **duplicate-sale bug** (double-click Complete Sale saved 2-3 orders + deducted stock each time; fixed with a ref guard + backend 10s identical-order check); `NumberField` (no default 0 in line-item price/cost); line items: per-line Category/Manufacturer filter, drag reorder (new `sort_order` on estimate/invoice/job_line_items, all inserts/selects ordered), Notes textarea; Document List (`document_types` config list, Data page card) + document rename/delete on Job/Estimate + empty Documents card collapses; Schedule: Tasks/Estimates/Techs-with-jobs boxes, estimates in Unassigned with quick-view popup, resizable panel/Other row/grid (localStorage `schedule-sizes`); Route order dialog drag + Permanent/Temporary (`permanent:false` = only re-time this day's real jobs); Jobs top filters default to signed-in tech + today with Today/Week/Month (auto-defaults ignored on Schedule/Dispatch/Map until changed by hand); Timesheets rebuilt on new `time_entries`/`time_requests` tables (persisted clock in/out, edit/add entries with notes, requests approve/deny; ClockCard also on Field). 3 additive migrations applied to prod (receipt_disclaimer, line-item sort_order, time_entries+time_requests). Same day: Documents section on InvoiceDetail too (new `invoice_attachments` table, print:hidden). Open: 6 duplicate POS orders from the bug still in prod (CLIENT_REQUESTS.md #9) — cleanup needs the dev's explicit OK.
