...
export function receiptHtml(r: ReceiptData, b: ReceiptBusiness): string {
  const rows = r.lines.map((l) => `
    <tr><td colspan="3" class="name">${esc(l.name)}${l.qty < 0 ? " (Return)" : ""}</td></tr>
...
<style>
  ...
  <h1>${esc(b.businessName || "Receipt")}</h1>
  ${b.address ? `<div class="c">${esc(b.address)}</div>` : ""}
  ${b.phone ? `<div class="c">${esc(b.phone)}</div>` : ""}
  <hr>
  <div>Receipt: <b>${esc(r.number)}</b></div>
  <div>Date: ${esc(r.date.toLocaleString())}</div>
  <div>Customer: ${esc(r.customer)}</div>
  <hr>
  <table>${rows}</table>
  <hr>
  <table class="tot">
    <tr><td>Subtotal</td><td class="r">${money(r.subtotal)}</td></tr>
    ${r.discount ? `<tr><td>Discount</td><td class="r">-${money(r.discount)}</td></tr>` : ""}
    <tr><td>Tax</td><td class="r">${money(r.tax)}</td></tr>
    <tr class="grand"><td>Total</td><td class="r">${money(r.total)}</td></tr>
    <tr><td>Paid by</td><td class="r">${esc(r.payment)}</td></tr>
  </table>
...
export function printReceipt(r: ReceiptData, b: ReceiptBusiness) {
  const w = window.open("", "_blank", "width=420,height=640");
  if (!w) return false;
  w.document.open();
  w.document.write(receiptHtml(r, b));
  w.document.close();
  w.focus();
